...
show = ['users', 'groups', 'computers', 'pwdpolicy', 'store', 'creds', 'fgpolicy', 'file servers', 'access', 'SPN']
...
class menu(cmd.Cmd):
	...
	try:
		...
		def do_show(self, option, intro=None):
			try:
				...
				if option == 'computers':
					...
					return cmd.Cmd.cmdloop(self, intro)
				if option == 'SPN':
					tb = dp.read_sql('select * from SPNTB', conn)
					print((tabulate(tb, showindex=False, headers=tb.columns, tablefmt="grid")))
					result(tb)
					return cmd.Cmd.cmdloop(self, intro)
				if option == 'store':
		...
		def do_export(self, option, intro=None):
			try:
				...
				if path == '':
					...
					return
				tablesnames = ['User','Group', 'Computer', 'SPN']
				if format =="csv":
			...
			try:
				...
				if type in ["user"]:
					...
					for c1, c2, c3 in zip(display_Members_Of[::3], display_Members_Of[1::3], display_Members_Of[2::3]):
						...
						print('{:<30}{:<30}{:<}'.format(c1.decode('utf-8'), c2.decode('utf-8'), c3.decode('utf-8')))
					result(tb)
					return cmd.Cmd.cmdloop(self, intro)
				elif type in ["group"]:
		...
		def do_list(self, option, intro=None):
			try:
				...
				if "computers" in option:
					...
					return cmd.Cmd.cmdloop(self, intro)
				if "spns" in option:
					tb = dp.read_sql('select Username from SPNTB', conn)
					SPNs = tabulate(tb, showindex=False)
					print(SPNs.strip())
					result(SPNs)
					if "-f" in option or "--file" in option:
						option = option.split(" ")
						filepath = option[2]
						tb.to_csv(filepath, sep='\t', index=False)
						print(colors.GRN + "[+] " + colors.NRM + "Exported List of SPNS to: " + filepath)
					return cmd.Cmd.cmdloop(self, intro)		
			except pandas.io.sql.DatabaseError:
				print(colors.RD + "[-] " + colors.NRM + "Error: Empty Database.")
		...
		def do_query(self, option, intro=None):
			try:
				...
				if 'Users' in option:
					...
					option = option.replace("SPNS", "SPNTB")
				elif 'spns' in option:
					option = option.replace("spns", "SPNTB")
				elif 'SPN' in option:
					option = option.replace("SPN", "SPNTB")
				elif 'spn' in option:
					option = option.replace("spn", "SPNTB")
				else:
		...
		def do_search(self, option, intro=None):
			try:
				...
				if computer_tb.empty:
					...
					print("Computers")
					print("---------")
					print((tabulate(computer_tb, showindex=False, headers=spn.columns, tablefmt="grid")))
				spn_tb = dp.read_sql('select * from SPNTB where SPN like "%' + value + '%" or Username like "%' + value + '%" or Description like "%' + value + '%" or "Member Of" like "%' + value + '%" COLLATE NOCASE', conn)
				if spn_tb.empty:
					pass
				else:
					print("SPNs")
					print("---------")
					print((tabulate(spn_tb, showindex=False, headers=spn_tb.columns, tablefmt="grid")))
				return cmd.Cmd.cmdloop(self, intro)
			except KeyboardInterrupt:
				return
			return cmd.Cmd.cmdloop(self, intro)
		...
		def do_help(self, intro=None):
			...
			print("export               Export the contents of the database to a path in one of the following formats: csv, html. (using with -f or --filetype and -p or --path for the file path)")
